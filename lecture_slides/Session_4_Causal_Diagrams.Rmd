...
<div class="header-container first-slide-header">
  ...
  <h1>Lecture 4: Causal Diagrams</h1> 
</div>
...
- There's a theoretical concept we're trying to get at, and we want to isolate *just the variation* that represents that effect of interest
...
- (if it's possible; sometimes the model says that it is impossible to identify an effect)
- Just like Arteaga did, if we know a source of identified variation in how `X` was assigned, and using that information we are able to identify the true treatment effect
- (we might still run into estimation problems, e.g., measurement error in one of the variables or a *small* sample)
...
df <- tibble(tech = sample(c(0,1),5000,replace=T,prob=c(.7,.3))) %>%
  #Tech firms on average spend $3mil more defending IP lawsuits
  mutate(IP.spend = 3*tech+runif(5000,min=0,max=4)) %>%
  #Tech firms also have higher profits. But IP lawsuits lower profits
  mutate(log.profit = 2*tech - .3*IP.spend + rnorm(5000,mean=2))
...
```{r, echo=FALSE, fig.width=10, fig.height=5.5, out.width="100%", fig.align="center"}
library(tidyverse)
library(gganimate)
library(gifski)
...
- Our model builds on assumptions that we derive from our insights
- 
...
- Remember, "cause" just means "changes the probability of". Not all people who climb mountains are Tyroleans, but being Tyrolean increases the probability of climbing mountains, so a graph of climbing mountains would have $Tyrolean \rightarrow ClimbingMountains$ on it
...
- Don't draw the DAG just so it will be easy to identify. Drawing the DAG does not make it so! The goal is to reflect the true model of the world
...
- Variables that affect the outcome but aren't related to anything else aren't really important (you'll see why later)
...
- The arrows you *leave out* are important to think about - are you sure there's no effect? You need those NON-arrows to be able to causally identify anything.
